...
foreach ($files as $file) {
    ...
    $sqld = mysqli_query($sqlconn,"SELECT user.nama, sidanira.nisn FROM user LEFT JOIN sidanira ON (user.userid = sidanira.nis OR user.reg_ppdb = sidanira.nis) WHERE user.nama = '$nama_esc' OR sidanira.nama = '$nama_esc' LIMIT 1");
    $d    = mysqli_fetch_array($sqld);

    if ($d) {
        $nisn_db = $d['nisn'];
        $nama_db = $d['nama']; // Nama asli dari Database
